...
WITH CTE
  AS (
    SELECT  BankAccountKey
          ...
          , TransDateKey
          , TransAmount
      FROM {{ ref("d365cma_bankaccounttrans_f") }}
    UNION ALL
    SELECT  BankAccountKey
          , bk.LegalEntityKey
          , TransDateKey
      ...
      FROM (   SELECT  d.DateKey AS TransDateKey
                    , 0.00      AS TransAmount
                  FROM {{ ref('d365cma_date_d') }} d
                WHERE d.DateKey > (SELECT  MIN(TransDateKey) FROM {{ ref("d365cma_bankaccounttrans_f") }})
                  AND d.DateKey < (SELECT  MAX(TransDateKey) FROM {{ ref("d365cma_bankaccounttrans_f") }})
                  AND d.DateKey NOT IN ( SELECT  DISTINCT TransDateKey FROM {{ ref("d365cma_bankaccounttrans_f") }} )) dt
      CROSS JOIN (SELECT  DISTINCT BankAccountKey, LegalEntityKey FROM {{ ref("d365cma_bankaccounttrans_f") }})         bk )
...
ORDER BY sub.TransDate)              AS [Closing balance]
            ...
            FROM (   SELECT  c1.BankAccountKey
                        ...
                        FROM CTE           c1
                      INNER JOIN {{ ref('d365cma_date_d') }} d1
                      ...
                      SELECT  c2.BankAccountKey
                        ...
                        FROM CTE           c2
                      INNER JOIN {{ ref('d365cma_date_d') }} d2
            ...
            INNER JOIN {{ ref('d365cma_date_d') }}                                                  d3
              ON d3.Date                = sub.TransDate
            LEFT JOIN {{ ref("d365cma_ledgertranstype_d") }}                                       ltt
...
SELECT  BankAccountKey AS [Bank account key]
  ...
  FROM {{ ref("d365cma_bankaccounttrans_f") }} F
  LEFT JOIN {{ ref("d365cma_ledgertranstype_d") }}  ltt
