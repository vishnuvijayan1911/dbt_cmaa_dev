...
FROM (   SELECT  le.LegalEntityKey                      AS [Legal entity key]
          ...
          FROM {{ ref("d365cma_salesorderline_f") }} solf
          INNER JOIN {{ ref("d365cma_salesorderline_d") }} sol
          ...
          INNER JOIN {{ ref("d365cma_paymentterm_d") }}    pt
            ON pt.PaymentTermKey     = solf.PaymentTermKey
          INNER JOIN {{ ref("d365cma_salesstatus_d") }}    st
            ON st.SalesStatusKey     = solf.SalesLineStatusKey
          INNER JOIN {{ ref("d365cma_customer_d") }}       dc
            ON dc.CustomerKey        = solf.CustomerKey
          LEFT JOIN {{ ref('d365cma_date_d') }}           dt
            ON dt.Date               = CAST(GETDATE() AS DATE)
          LEFT JOIN {{ ref('d365cma_date_d') }}           dt1
        ...
        SELECT  le.LegalEntityKey                       AS [Legal entity key]
          ...
          FROM {{ ref("d365cma_purchaseorderline_f") }} polf
          ...
          INNER JOIN {{ ref("d365cma_purchasestatus_d") }}    st
    ...
    UNION
        SELECT  bt.LegalEntityKey   AS [Legal entity key]
          ...
          FROM {{ ref("d365cma_glbudgettrans_f") }}  bt
          INNER JOIN {{ ref("d365cma_legalentity_d") }}    le
            ON le.LegalEntityKey = bt.LegalEntityKey
          LEFT JOIN {{ ref("d365cma_ledgeraccount_d") }}  la
        ...
        SELECT  sf.LegalEntityKey  AS [Legal entity key]
          ...
          FROM {{ ref("d365cma_salesforecast_f") }}  sf
           INNER JOIN {{ ref("d365cma_legalentity_d") }}    le
        ...
        SELECT  pf.LegalEntityKey      AS [Legal entity key]
          ...
          FROM {{ ref("d365cma_purchaseforecast_f") }} pf
           INNER JOIN {{ ref("d365cma_legalentity_d") }}    le
