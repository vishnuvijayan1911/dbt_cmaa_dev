...
WITH CTE
  AS (
    SELECT  t.CustomerKey
      ...
      FROM {{ ref("d365cma_customer_d") }}             t
      LEFT JOIN {{ ref("d365cma_salesorder_f") }} sil
...
SELECT  t.CustomerKey                                                                                   AS [Customer key]
  ...
  FROM {{ ref("d365cma_customer_d") }}            t 
  LEFT JOIN {{ ref("d365cma_customer_f") }}  F 
    ON F.CustomerKey         = t.CustomerKey
  LEFT JOIN {{ ref("d365cma_address_d") }}        A 
    ON A.AddressKey          = F.AddressKey
  LEFT JOIN {{ ref("d365cma_customergroup_d") }}  g 
