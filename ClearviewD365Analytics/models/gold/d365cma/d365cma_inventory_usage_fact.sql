...
SELECT  pd.DateKey                                                                  AS [Usage date key]
  ...
  FROM {{ ref("d365cma_inventorytrans_f") }}       it
INNER JOIN {{ ref("d365cma_inventorysource_d") }}      s
    ON s.InventorySourceKey       = it.InventorySourceKey
INNER JOIN {{ ref("d365cma_inventory_trans_status_d") }} ts
    ON ts.InventoryTransStatusKey = it.InventoryTransStatusKey
INNER JOIN {{ ref('d365cma_date_d') }}                 pd
