...
WITH cte
  AS (
    SELECT  pt.ProductionKey
      ...
      FROM {{ ref("d365cma_productionpicklistjournal_f") }} pt 
      LEFT JOIN {{ ref("d365cma_production_d") }}                f 
  ...
  AS (SELECT  pt.ProductionKey
        ...
        FROM {{ ref("d365cma_productionfinishedjournal_f") }} pt 
        LEFT JOIN {{ ref("d365cma_production_d") }}                f 
          ON f.ProductionKey = pt.ProductionKey
      GROUP BY pt.ProductionKey)
  , cte2
  ...
  FROM {{ ref("d365cma_productioncoproduct_f") }}
...
SELECT  t.ProductionKey                                                                     AS [Production key]
  ...
  FROM {{ ref("d365cma_production_f") }}       t 
  LEFT JOIN {{ ref("d365cma_production_d") }}       f 
