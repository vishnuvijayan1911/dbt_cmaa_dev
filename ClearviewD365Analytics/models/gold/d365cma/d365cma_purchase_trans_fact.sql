...
SELECT  ROW_NUMBER() OVER (ORDER BY COALESCE(poltf.PurchaseOrderLineTransKey, -1)
  ...
  FROM {{ ref("d365cma_purchaseorderlinetrans_f") }}              poltf 
INNER JOIN {{ ref("d365cma_purchaseorderline_f") }}              polf 
    ON polf.PurchaseOrderLineKey        = poltf.PurchaseOrderLineKey
  LEFT JOIN {{ ref('d365cma_date_d') }}                                pold 
    ON pold.DateKey                     = polf.OrderDateKey
  FULL OUTER JOIN {{ ref("d365cma_productreceiptlinetrans_f") }}  prltf 
    ON prltf.PurchaseOrderLineTransKey  = poltf.PurchaseOrderLineTransKey
  LEFT JOIN {{ ref("d365cma_productreceiptline_f") }}             prlf 
    ON prlf.ProductReceiptLineKey       = prltf.ProductReceiptLineKey
  LEFT JOIN {{ ref('d365cma_date_d') }}                                prld 
    ON prld.DateKey                     = prlf.ReceiptDateKey
  FULL OUTER JOIN {{ ref("d365cma_purchaseinvoicelinetrans_f") }} piltf 
    ON piltf.ProductReceiptLineTransKey = prltf.ProductReceiptLineTransKey
  LEFT JOIN {{ ref("d365cma_purchaseinvoiceline_f") }}            pilf 
    ON pilf.PurchaseInvoiceLineKey      = piltf.PurchaseInvoiceLineKey
  LEFT JOIN {{ ref('d365cma_date_d') }}                                pild 
    ON pild.DateKey                     = pilf.InvoiceDateKey
  LEFT JOIN {{ ref("d365cma_inventory_trans_status_d") }}                ist 
  ...
  FULL OUTER JOIN {{ ref("d365cma_purchaserequisitionline_f") }}  prl 
    ON prl.PurchaseRequisitionLineKey   = polf.PurchaseRequisitionLineKey
  LEFT JOIN {{ ref('d365cma_date_d') }}                                Receiveddate
    ON Receiveddate.DateKey             = prlf.ReceiptDateKey
  LEFT JOIN {{ ref('d365cma_date_d') }}                                prd 
    ON prd.DateKey                      = prl.CreatedDateKey
  LEFT JOIN {{ ref("d365cma_salesorderline_f") }}                 solf
