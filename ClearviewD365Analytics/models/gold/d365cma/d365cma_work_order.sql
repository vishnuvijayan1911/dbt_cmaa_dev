...
SELECT  t.WorkOrderKey                                                                                      AS [Work order key]
  ...
  FROM {{ ref("d365cma_workorder_d") }}                 t 
INNER JOIN {{ ref("d365cma_workorder_f") }}       f 
    ON f.WorkOrderKey            = t.WorkOrderKey
  LEFT JOIN {{ ref("d365cma_workergroup_d") }}          wg
    ON wg.WorkerGroupKey         = f.WorkerGroupKey
  LEFT JOIN {{ ref("d365cma_workordercriticality_d") }} c 
    ON c.WorkOrderCriticalityKey = f.WorkOrderCriticalityKey
  LEFT JOIN {{ ref("d365cma_workordertype_d") }}        wot 
    ON wot.WorkOrderTypeKey      = f.WorkOrderTypeKey
  LEFT JOIN {{ ref("d365cma_workorderstate_d") }}       wos 
