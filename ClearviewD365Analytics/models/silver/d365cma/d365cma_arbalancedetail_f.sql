...
arbalancedetail_factdate445 AS (
    SELECT DISTINCT
      ...
      FROM {{ ref('d365cma_date_d') }}   dd
      INNER JOIN arbalancedetail_factdate t
         ON dd.FiscalMonthDate = t.FiscalMonthDate
      CROSS JOIN (SELECT FiscalMonthDate FROM {{ ref('d365cma_date_d') }} WHERE FiscalDate = CAST (GETDATE () AS DATE)) cte1
