{{ config(materialized='table', tags=['silver'], alias='caseassociation_fact') }}

-- Source file: cma/cma/layers/_base/_silver/caseassociation_f/caseassociation_f.py
...
SELECT {{ dbt_utils.generate_surrogate_key(['ts._RecID', 'ts._SourceID']) }} AS CaseAssociationKey
  ...
  LEFT JOIN {{ ref('d365cma_case_d') }}                      cd1
    ON cd1.CaseID          = ts.RefCaseID
  LEFT JOIN {{ ref('d365cma_vendor_d') }}                    v
    ON v.LegalEntityID     = ts.LegalEntityID
   AND v.VendorAccount     = ts.VendorAccount
  LEFT JOIN {{ ref('d365cma_purchaseorder_d') }}             po
  ...
  LEFT JOIN {{ ref('d365cma_purchaseinvoice_d') }}           dpi
  ...
  LEFT JOIN {{ ref('d365cma_purchaseinvoiceline_d') }}       pil
    ON pil._RecID2         = ts.RecID_VIT
  LEFT JOIN {{ ref('d365cma_salesorder_d') }}                so1
    ON so1.LegalEntityID   = ts.LegalEntityID
   AND so1._RecID          = ts.RecID_ReturnItem
  LEFT JOIN {{ ref('d365cma_qualityorder_d') }}              qo
