...
SELECT {{ dbt_utils.generate_surrogate_key(['le.LegalEntityKey', 'dca.LedgerAccountKey', 'dd.DateKey', 'bt.budgettypekey']) }} AS GLBudgetTransKey
     ...
     INNER JOIN {{ ref('d365cma_ledgeraccount_d') }}     dca
      ...
      LEFT JOIN {{ ref('d365cma_budgettransstatus_d') }} bts
        ON bts.BudgetTransStatusID = te.BudgetTransStatusID
      LEFT JOIN {{ ref('d365cma_budgettranstype_d') }}   btt
        ON btt.BudgetTransTypeID   = te.BudgetTransTypeID
      LEFT JOIN {{ ref('d365cma_budgettype_d') }}        bt
        ON bt.budgettypeid         = te.BudgetTypeID
      LEFT JOIN {{ ref('d365cma_currency_d') }}          tc
