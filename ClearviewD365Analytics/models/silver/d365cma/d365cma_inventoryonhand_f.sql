{{ config(materialized='table', tags=['silver'], alias='inventoryonhand_fact') }}

-- Source file: cma/cma/layers/_base/_silver/inventoryonhand_f/inventoryonhand_f.py
...
inventoryonhand_factstage AS (
    ...
    INNER JOIN {{ ref('inventdim') }}   id
        ON id.dataareaid        = oi.dataareaid
      AND id.inventdimid        = oi.inventdimid
      --INNER JOIN {{ ref('d365cma_inventsumavailablephysical') }} osi
...
inventoryonhand_factdetailmain AS (
    ...
    INNER JOIN {{ ref('d365cma_legalentity_d') }}          le
      ...
      LEFT JOIN {{ ref('d365cma_product_d') }}              dp2
      ...
      LEFT JOIN {{ ref('d365cma_warehouselocation_d') }}    dwl1
      ...
      LEFT JOIN {{ ref('d365cma_tag_d') }}                  dt2
