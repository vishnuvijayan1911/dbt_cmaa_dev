...
inventorytrans_factdetailmain AS (
    SELECT le.LegalEntityKey                    AS LegalEntityKey
         ...
         INNER JOIN {{ ref('d365cma_legalentity_d') }}          le
          ...
          LEFT JOIN {{ ref('d365cma_lot_d') }}                  il
          ...
          LEFT JOIN {{ ref('d365cma_salesorderline_d') }}       fsl
            ON fsl._RecID                     = t1.RecID_SL
           AND fsl._SourceID                  = 1
          LEFT JOIN {{ ref('d365cma_purchaseorderline_d') }}    dpl
          ...
          LEFT JOIN {{ ref('d365cma_voucher_d') }}              vou1
