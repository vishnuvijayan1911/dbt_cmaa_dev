...
nonconformance_facttmp AS (
    SELECT nc.nonconformancekey       AS NonConformanceKey
     ...
     INNER JOIN {{ ref('d365cma_nonconformance_d') }}    nc
     ...
     INNER JOIN {{ ref('d365cma_date_d') }}              dd
      ...
      LEFT JOIN {{ ref('d365cma_inventorysite_d') }}     vs
      ...
      LEFT JOIN {{ ref('d365cma_production_d') }}        dpo
        ON dpo.LegalEntityID  = t1.LegalEntityID
       AND dpo.ProductionID   = t1.ProductionID
      LEFT JOIN {{ ref('d365cma_salesperson_d') }}       dsp1
        ON dsp1._RecID        = t1.WorkerResponsible
      LEFT JOIN {{ ref('d365cma_salesperson_d') }}       dsp2
        ON dsp2._RecID        = t1.ReportedBy
      LEFT JOIN {{ ref('d365cma_salesperson_d') }}       dsp3
