{{ config(materialized='table', tags=['silver'], alias='packingsliplinetrans_fact') }}

-- Source file: cma/cma/layers/_base/_silver/packingsliplinetrans_f/packingsliplinetrans_f.py
...
packingsliplinetrans_facttrans AS (
    ...
    ORDER BY ISNULL(ts.RecID_IT, 0)) = 1
          ...
          FROM {{ ref('d365cma_packingslipline_f') }} frl
...
packingsliplinetrans_factadj AS (
    ...
    INNER JOIN {{ ref('d365cma_packingslipline_f') }} fcl
...
SELECT DISTINCT 
  ...
  LEFT JOIN {{ ref('d365cma_inventory_trans_status_d') }}     dis
  ...
  LEFT JOIN {{ ref('d365cma_salesorderlinetrans_f') }} solt
