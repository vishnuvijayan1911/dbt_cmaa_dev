{{ config(materialized='table', tags=['silver'], alias='productioncoproduct_fact') }}

-- Source file: cma/cma/layers/_base/_silver/productioncoproduct_f/productioncoproduct_f.py
...
productioncoproduct_factdetail1 AS (
    SELECT pp.ProductionCoProductKey AS ProductionCoProductKey
         ...
         INNER JOIN {{ ref('d365cma_productioncoproduct_d') }} pp
          ...
          LEFT JOIN {{ ref('d365cma_production_d') }}          po
