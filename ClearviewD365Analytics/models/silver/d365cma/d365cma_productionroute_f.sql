...
SELECT
     ...
     INNER JOIN {{ ref('d365cma_productionroute_d') }}          pb
      ...
      LEFT JOIN {{ ref('d365cma_productionresource_d') }}       res
        ON res.LegalEntityID           = t1.LegalEntityID
       AND res.ResourceID              = t1.ResourceID
      LEFT JOIN {{ ref('d365cma_productionrouteoperation_d') }} ro
        ON ro.LegalEntityID            = t1.LegalEntityID
       AND ro.OperationID              = t1.OperationID
      LEFT JOIN {{ ref('d365cma_productionroutegroup_d') }}     dprg
