{{ config(materialized='table', tags=['silver'], alias='purchaseforecast_fact') }}

-- Source file: cma/cma/layers/_base/_silver/purchaseforecast_f/purchaseforecast_f.py
        ...
        FROM purchaseforecast_factstage                ts
         INNER JOIN {{ ref('d365cma_legalentity_d') }}   le
          ...
          LEFT JOIN {{ ref('d365cma_financial_d') }}     fd
           ...
           AND fd._SourceID       = 1
          LEFT JOIN {{ ref('d365cma_forecastmodel_d') }} dm
          ...
          LEFT JOIN {{ ref('d365cma_inventorysite_d') }} iv
