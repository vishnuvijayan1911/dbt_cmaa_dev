{{ config(materialized='table', tags=['silver'], alias='purchaseinvoiceline_fact') }}

-- Source file: cma/cma/layers/_base/_silver/purchaseinvoiceline_f/purchaseinvoiceline_f.py
...
purchaseinvoiceline_factcharge AS (
    SELECT dvil.PurchaseInvoiceLineKey          AS PurchaseInvoiceLineKey
          ...
          FROM {{ ref('d365cma_purchaseinvoicelinecharge_f') }} crg


         INNER JOIN {{ ref('d365cma_purchaseinvoiceline_d') }}       dvil
...
purchaseinvoiceline_factline AS (
    SELECT dvil.PurchaseInvoiceLineKey                                                                                AS PurchaseInvoiceLineKey
         ...
         INNER JOIN {{ ref('d365cma_purchaseinvoiceline_d') }} dvil
          ...
          LEFT JOIN {{ ref('d365cma_uom_d') }}                 pur
...
SELECT DISTINCT tl.PurchaseInvoiceLineKey
    ...
    -- AND vuc1.touom          = 'PC'
      ...
      LEFT JOIN {{ ref('d365cma_vwuomconversion_ft') }} vuc3
