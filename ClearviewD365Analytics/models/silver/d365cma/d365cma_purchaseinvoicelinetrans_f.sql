{{ config(materialized='table', tags=['silver'], alias='purchaseinvoicelinetrans_fact') }}

-- Source file: cma/cma/layers/_base/_silver/purchaseinvoicelinetrans_f/purchaseinvoicelinetrans_f.py
...
purchaseinvoicelinetrans_facttrans AS (
    ...
    ORDER BY ISNULL (tr.RecID_IT, 0)) = 1
          ...
          FROM {{ ref('d365cma_purchaseinvoiceline_f') }} fcl
...
purchaseinvoicelinetrans_factadj AS (
    SELECT t.PurchaseInvoiceLineKey
         ...
         INNER JOIN {{ ref('d365cma_purchaseinvoiceline_f') }} fcl
...
purchaseinvoicelinetrans_factinvoicetrans AS (
    ...
    ORDER BY polt._RecID2) AS OrderTransRank
          ...
          FROM {{ ref('d365cma_purchaseinvoiceline_f') }}         frl

         INNER JOIN {{ ref('d365cma_purchaseorderlinetrans_f') }} polt
...
purchaseinvoicelinetrans_factinvoicetrans2 AS (
    ...
    ORDER BY prlt._RecID2) AS OrderTransRank
         ...
         INNER JOIN {{ ref('d365cma_productreceiptline_f') }}      prl
            ON prl.PurchaseOrderLineKey   = frl.PurchaseOrderLineKey
         INNER JOIN {{ ref('d365cma_productreceiptlinetrans_f') }} prlt
...
SELECT {{ dbt_utils.generate_surrogate_key(['fcl._RecID1', 'fcl._RecID2', 'tt.RecID_IT']) }} AS PurchaseInvoiceLineTransKey
      ...
      LEFT JOIN {{ ref('d365cma_productreceiptlinetrans_f') }} prlt
