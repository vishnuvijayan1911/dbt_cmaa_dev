...
purchaseorderline_factcharge AS (
    SELECT pol.PurchaseOrderLineKey            AS PurchaseOrderLineKey
          ...
          FROM {{ ref('d365cma_purchaseorderlinecharge_f') }} crg
         INNER JOIN {{ ref('d365cma_purchaseorderline_d') }}       pol
...
purchaseorderline_factdetailmain AS (
    SELECT dpol.PurchaseOrderLineKey
          ...
          FROM purchaseorderline_factstage                           ts
         INNER JOIN {{ ref('d365cma_purchaseorderline_d') }}       dpol
         ...
         INNER JOIN {{ ref('d365cma_legalentity_d') }}             le
          ...
          LEFT JOIN {{ ref('d365cma_ontimedeliverystatus_d') }}    ot
          ...
          LEFT JOIN {{ ref('d365cma_purchaseorder_d') }}           dpo
          ...
          LEFT JOIN {{ ref('d365cma_uom_d') }}                     du1
            ON du1.UOM                       = ts.PurchaseUnit
          LEFT JOIN {{ ref('d365cma_uom_d') }}                     du2
            ON du2.UOM                       = ts.ReceivingUnit
          LEFT JOIN {{ ref('d365cma_currency_d') }}                dc
          ...
          LEFT JOIN {{ ref('d365cma_returnstatus_d') }}            drs
            ON drs.ReturnStatusID            = ts.ReturnStatusID
          LEFT JOIN {{ ref('d365cma_returnreason_d') }}            drr
            ON drr.LegalEntityID             = ts.LegalEntityID
           AND drr.ReturnReasonID            = ts.ReturnReasonID
          LEFT JOIN {{ ref('d365cma_procurementcategory_d') }}     dpc
          ...
          LEFT JOIN {{ ref('d365cma_userinfo_d') }}                u
            ON u.LegalEntityID               = ts.LegalEntityID
           AND u.UserName                    = ts.Approver1
          LEFT JOIN {{ ref('d365cma_userinfo_d') }}                u1
          ...
          LEFT JOIN {{ ref('d365cma_employee_d') }}                de
           ...
           AND de._SourceID                  = 1
          LEFT JOIN {{ ref('d365cma_purchaseagreementline_d') }} pal
...
SELECT DISTINCT td.PurchaseOrderLineKey
      ...
      LEFT JOIN {{ ref('d365cma_vwuomconversion_ft') }} vuc
    ...
    -- AND vuc4.touom          = 'SQIN'
      ...
      LEFT JOIN {{ ref('d365cma_vwuomconversion') }} vuc6
