...
purchaserequisitionline_factdetail1 AS (
    SELECT dprl.PurchaseRequisitionLineKey                         AS PurchaseRequisitionLineKey
         ...
         INNER JOIN {{ ref('d365cma_purchaserequisitionline_d') }} dprl
         ...
         INNER JOIN {{ ref('d365cma_legalentity_d') }}             le
          ...
          LEFT JOIN {{ ref('d365cma_buyergroup_d') }}              dbg
         ...
         INNER JOIN {{ ref('d365cma_date_d') }}                    dd
          ...
          LEFT JOIN {{ ref('d365cma_requisitionstatus_d') }}       drs
            ON drs.RequisitionStatusID  = ts.RequisitionLineStatusID
          LEFT JOIN {{ ref('d365cma_requisitionstatus_d') }}       drs1
            ON drs1.RequisitionStatusID = ts.RequisitionStatusID
          LEFT JOIN {{ ref('d365cma_requisitiontype_d') }}         drt
          ...
          LEFT JOIN {{ ref('d365cma_documentstate_d') }}           ds
