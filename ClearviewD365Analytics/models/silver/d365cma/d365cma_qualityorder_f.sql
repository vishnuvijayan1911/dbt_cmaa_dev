...
qualityorder_factdetail1 AS (
    SELECT dqo.QualityOrderKey             AS QualityOrderKey
     ...
     INNER JOIN {{ ref('d365cma_qualityorder_d') }}             dqo
      ...
      LEFT JOIN {{ ref('d365cma_purchaseorderline_d') }}        dpol
      ...
      LEFT JOIN {{ ref('d365cma_vendor_d') }}                   dv
       ...
       AND dv._SourceID        = 1
      LEFT JOIN {{ ref('d365cma_production_d') }}               dp1
        ON dp1._RecID          = ts.RecID_PT
       AND dp1._SourceID       = 1
      LEFT JOIN {{ ref('d365cma_productioncoproduct_d') }}      dpc
        ON dpc._RecID          = ts.RecID_PC
       AND dpc._SourceID       = 1
      LEFT JOIN {{ ref('d365cma_productionroute_d') }}          dpr
        ON dpr._RecID          = ts.RecID_PR
       AND dpr._SourceID       = 1
      LEFT JOIN {{ ref('d365cma_productionresource_d') }}       pr
