...
SELECT dqol.QualityOrderLineKey        AS QualityOrderLineKey
 ...
 INNER JOIN {{ ref('d365cma_qualityorderline_d') }}         dqol
  ...
  LEFT JOIN {{ ref('d365cma_qualitytest_d') }}                     dt
    ON dt.LegalEntityID   = ts.LegalEntityID
   AND dt.TestID          = ts.TestID
  LEFT JOIN {{ ref('d365cma_uom_d') }}                      uom
