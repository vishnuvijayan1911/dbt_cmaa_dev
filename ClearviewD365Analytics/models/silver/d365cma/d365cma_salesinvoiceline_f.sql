...
salesinvoiceline_factcharge AS (
    SELECT sil.SalesInvoiceLineKey            AS SalesInvoiceLineKey
      ...
      FROM {{ ref('d365cma_salesinvoicelinecharge_f') }} crg
...
salesinvoiceline_factmain AS (
    ...
    INNER JOIN {{ ref('d365cma_salesinvoiceline_d') }}          dcil
        ON dcil._RecID1           = ts._RecID1
      AND dcil._RecID2           = ts._RecID2
      AND dcil._SourceID         = 1
    INNER JOIN {{ ref('d365cma_salesinvoice_d') }}              si
        ON si._RecID              = ts._RecID1
      AND si._SourceID           = 1
      LEFT JOIN {{ ref('d365cma_inventorysite_d') }}             dp
      ...
      LEFT JOIN {{ ref('d365cma_salesorderline_d') }}            dsil
        ON dsil._RecID            = ts.RecID_SL
      AND dsil._SourceID         = 1
      LEFT JOIN {{ ref('d365cma_product_d') }}                   dr
      ...
      LEFT JOIN {{ ref('d365cma_salestype_d') }}                 sit
      ...
      AND dsc._SourceID          = 1
      LEFT JOIN {{ ref('d365cma_invoicetype_d') }}               ivt
