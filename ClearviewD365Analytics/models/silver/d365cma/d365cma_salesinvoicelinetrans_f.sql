{{ config(materialized='table', tags=['silver'], alias='salesinvoicelinetrans_fact') }}

-- Source file: cma/cma/layers/_base/_silver/salesinvoicelinetrans_f/salesinvoicelinetrans_f.py
...
salesinvoicelinetrans_facttrans AS (
    SELECT fcl.SalesInvoiceLineKey                                                             AS SalesInvoiceLineKey
       ...
       ORDER BY ISNULL (it.recid, 0)) = 1
             ...
             FROM {{ ref('d365cma_salesinvoiceline_f') }} fcl
...
salesinvoicelinetrans_factadj AS (
    SELECT t.SalesInvoiceLineKey
          ...
          INNER JOIN {{ ref('d365cma_salesinvoiceline_f') }} fcl
...
salesinvoicelinetrans_factinvoicetrans AS (
    ...
    FROM {{ ref('d365cma_salesinvoiceline_f') }}         frl
    INNER JOIN {{ ref('d365cma_salesorderlinetrans_f') }} polt
...
salesinvoicelinetrans_factinvoicetrans2 AS (
    ...
    FROM {{ ref('d365cma_salesinvoiceline_f') }}          frl
    INNER JOIN {{ ref('d365cma_packingslipline_f') }}      prl
       ON prl.SalesOrderLineKey   = frl.SalesOrderLineKey
    INNER JOIN {{ ref('d365cma_packingsliplinetrans_f') }} prlt
