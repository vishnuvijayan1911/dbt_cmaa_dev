...
salesorder_factlinecharge AS (
    SELECT sil._RecID                           AS RECID_SL
          ...
          FROM {{ ref('d365cma_salesorderlinecharge_f') }} crg
         INNER JOIN {{ ref('d365cma_salesorderline_d') }}       sil
...
salesorder_factdetailmain AS (
    SELECT dso.SalesOrderKey                                                         AS SalesOrderKey
         ...
         INNER JOIN {{ ref('d365cma_salesorder_d') }}        dso
         ...
         INNER JOIN {{ ref('d365cma_legalentity_d') }}       le
          ...
          LEFT JOIN {{ ref('d365cma_customergroup_d') }}     dcg
          ...
          LEFT JOIN {{ ref('d365cma_salesstatus_d') }}       dss
            ON dss.SalesStatusID    = ts.SalesStatusID
          LEFT JOIN {{ ref('d365cma_salestype_d') }}         dst
          ...
          LEFT JOIN {{ ref('d365cma_paymentterm_d') }}       dpa
          ...
          LEFT JOIN {{ ref('d365cma_returnreason_d') }}      rsi
          ...
          LEFT JOIN {{ ref('d365cma_deliveryreason_d') }}    ddr
