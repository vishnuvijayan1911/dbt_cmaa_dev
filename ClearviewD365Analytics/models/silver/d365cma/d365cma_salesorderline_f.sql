{{ config(materialized='table', tags=['silver'], alias='salesorderline_fact') }}

-- Source file: cma/cma/layers/_base/_silver/salesorderline_f/salesorderline_f.py
...
salesorderline_factcharge AS (
    SELECT sol.SalesOrderLineKey               AS SalesOrderLineKey
          ...
          FROM {{ ref('d365cma_salesorderline_d') }}                 sol
         INNER JOIN {{ ref('d365cma_salesorderlinecharge_f') }} crg
...
salesorderline_factdetailmain AS (
    SELECT DISTINCT sod.SalesOrderLineKey                                                       AS SalesOrderLineKey
          ...
          FROM salesorderline_factstage                      ts
         INNER JOIN {{ ref('d365cma_salesorderline_d') }}     sod
            ON sod._RecID            = ts.RECID
           AND sod._SourceID         = 1
         INNER JOIN {{ ref('d365cma_legalentity_d') }}        le
            ON le.LegalEntityID      = ts.LegalEntityID
          LEFT JOIN {{ ref('d365cma_salesagreementline_d') }} sal
          ...
          LEFT JOIN {{ ref('d365cma_date_d') }}               dd7
            ON dd7.Date              = CAST(GETDATE() AS DATE)
          LEFT JOIN {{ ref('d365cma_date_d') }}               dd8
          ...
          LEFT JOIN {{ ref('d365cma_salesstatus_d') }}        ps
            ON ps.SalesStatusID      = ts.SalesStatusID
          LEFT JOIN {{ ref('d365cma_salesstatus_d') }}        ps1
            ON ps1.SalesStatusID     = ts.SalesLineStatusID
          LEFT JOIN {{ ref('d365cma_salestype_d') }}          pt
            ON pt.SalesTypeID        = ts.SalesTypeID
          LEFT JOIN {{ ref('d365cma_ontimeshipstatus_d') }}   ot
          ...
          LEFT JOIN {{ ref('d365cma_paymentterm_d') }}        pyt
          ...
          LEFT JOIN {{ ref('d365cma_uom_d') }}                pku
            ON pku.UOM               = ts.PickingUnit
          LEFT JOIN {{ ref('d365cma_uom_d') }}                su
          ...
          LEFT JOIN {{ ref('d365cma_employee_d') }}           de2
            ON de2._RecID            = ts.SalesTaker
          LEFT JOIN {{ ref('d365cma_salescategory_d') }}      dsc
            ON dsc._RecID            = ts.RecID_SC
           AND dsc._SourceID         = 1
          LEFT JOIN {{ ref('d365cma_salesorder_d') }}         so
            ON so.LegalEntityID      = ts.LegalEntityID
           AND so.SalesOrderID       = ts.SalesOrderID
          LEFT JOIN {{ ref('d365cma_production_d') }}         prod
          ...
          LEFT JOIN {{ ref('d365cma_userinfo_d') }}           ui
...
salesorderline_factdetailbase AS (
    SELECT t.CurrencyKey                                                            AS CurrencyKey
          ...
          LEFT JOIN {{ ref('d365cma_legalentity_d') }}       le
