...
salesquoteline_factdetailmain AS (
    ...
    INNER JOIN {{ ref('d365cma_salesquoteline_d') }} dql
    ...
    INNER JOIN {{ ref('d365cma_legalentity_d') }}    le
      ...
      LEFT JOIN {{ ref('d365cma_quotestatus_d') }}    qs
        ON qs.QuoteStatusID    = ts.QuoteStatusID
      LEFT JOIN {{ ref('d365cma_quotetype_d') }}      qt
      ...
      LEFT JOIN {{ ref('d365cma_paymentterm_d') }}    dpt
        ON dpt.LegalEntityID   = ts.LegalEntityID
      AND dpt.PaymentTermID   = ts.PaymentTermID
      LEFT JOIN {{ ref('d365cma_paymentmode_d') }}    dpm
...
--  AND vuc4.touom          = 'SQIN'
  --  LEFT JOIN {{ ref('d365cma_vwuomconversion') }}   vuc5
