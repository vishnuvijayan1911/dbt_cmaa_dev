...
select  prd.productkey,
              ...
              CASE WHEN uomc.factor = 0 then CAST(0 as FLOAT)
		          ...
		          END as factor
      from {{ ref('d365cma_product_d') }} prd
      ...
      inner join {{ ref('unitofmeasure') }} touom on touom.recid = uomc.tounitofmeasure
	    inner join {{ ref('d365cma_uom_d') }} uom  on lower(fromuom.symbol) = lower(uom.uom)
	    inner join {{ ref('d365cma_uom_d') }} uom1 on lower(touom.symbol)   = lower(uom1.uom) 
