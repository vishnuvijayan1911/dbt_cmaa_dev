...
SELECT wo.WorkOrderKey
     ...
     INNER JOIN {{ ref('d365cma_workorder_d') }}            wo
        ON wo._RecID          = ts._RecID
       AND wo._SourceID       = 1
      LEFT JOIN {{ ref('d365cma_workordercriticality_d') }} woc
        ON woc._RecID         = ts.CRITICALITY
       AND woc._SourceID      = 1
      LEFT JOIN {{ ref('d365cma_workorderstate_d') }}       dws
        ON dws._RecID         = ts.WORKORDERLIFECYCLESTATE
       AND dws._SourceID      = 1
      LEFT JOIN {{ ref('d365cma_employee_d') }}             res
        ON res._RecID         = ts.RecID_HCMR
       AND res._SourceID      = 1
      LEFT JOIN {{ ref('d365cma_employee_d') }}             sch
      ...
      LEFT JOIN {{ ref('d365cma_workergroup_d') }}          dwg
        ON dwg._RecID         = ts.RESPONSIBLEWORKERGROUP
       AND dwg._SourceID      = 1
      LEFT JOIN {{ ref('d365cma_workordertype_d') }}        dwt
      ...
      LEFT JOIN {{ ref('d365cma_workorder_d') }}            wo1
        ON wo1.LegalEntityID  = le.LegalEntityID
       AND wo1.WorkOrderID    = ts.ParentWorkOrderID
      LEFT JOIN {{ ref('d365cma_on_time_status_d') }}         ond
        ON ond.OnTimeStatusID = ts.OnTimeStatusDueDate
      LEFT JOIN {{ ref('d365cma_on_time_status_d') }}         ons
