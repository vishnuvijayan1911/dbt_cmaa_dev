...
SELECT {{ dbt_utils.generate_surrogate_key(['ts._RecID', 'ts._SourceID']) }} AS WorkOrderLineKey
     ...
     INNER JOIN {{ ref('d365cma_legalentity_d') }}                    le
      ...
      LEFT JOIN {{ ref('d365cma_employee_d') }}                       w
        ON w._RecID                 = ts.RecID_HCM
       AND w._SourceID              = 1
      LEFT JOIN {{ ref('d365cma_maintenancejobtradecertificate_d') }} cer
