...
productproduct1 AS (
    SELECT DISTINCT
         ...
         , CASE WHEN it.cmarecoverablescrap = 'Yes'
                THEN 'Recoverable scrap'
                ELSE 'Not scrap'
           END                                                                       AS RecoverableScrap
         , 1                                                                         AS _SourceID
         , it.recid                                                                  AS _RecID
...
productvariant AS (
    SELECT DISTINCT
         ...
         , CAST(ISNULL(NULLIF(TRY_CAST(id.inventcolorid AS FLOAT), 0), 0)
             AS NUMERIC(32, 16))                                                       AS NumericLength
         , CAST(ISNULL(NULLIF(TRY_CAST(id.inventsizeid AS FLOAT), 0), 0)
             AS NUMERIC(32, 16))                                                       AS NumericWidth
...
productvariantunit AS (
    ...
    INNER JOIN {{ ref('ecoresproductdimensiongroupfldsetup') }} fs
        ON fs.productdimensiongroup = eg.recid
         AND CAST(fs.isactive AS VARCHAR(10)) = 'Yes' --fix 3
    GROUP BY tv.DATAAREAID, tv.ProductMasterID
),
productdetail1 AS (
    SELECT
           p1.BaseProduct
         ...
         , CASE WHEN pv.ProductID IS NULL
                THEN 'Product'
...
SELECT
     ...
     , pv.ProductCategory
     , pv.StorageGroupID
     , CASE WHEN pv.StorageGroup = '' THEN pv.StorageGroupID ELSE pv.StorageGroup END       AS StorageGroup
     , CASE WHEN pv.TrackingGroup = '' THEN pv.TrackingGroupID ELSE pv.TrackingGroup END    AS TrackingGroup
     , pv.TrackingGroupID
     , pv.WidthUnit
     , pv.RecoverableScrap
     , pv._SourceID
